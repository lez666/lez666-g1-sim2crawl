...
@configclass
class G1Crawl2ShambleSceneCfg(InteractiveSceneCfg):
  # ground terrain
...
class EventCfg:
    ...
    # reset
    # Curriculum-aware pose array reset with progressive difficulty (CRAWL2STAND)
...
@configclass
class CurriculumCfg:
    """Curriculum terms for progressive difficulty (CRAWL2STAND)."""
    
    # Expand pose array range BACKWARD from crawling to standing over training
    ...
    # Ramp up end anchor (standing) probability in final training stages
    end_anchor_difficulty = CurrTerm(
        func=mdp.modify_term_cfg,
        params={
            "address": "events.reset_robot.params.end_home_frame_prob",
...
@configclass
class G1Crawl2ShambleEnvCfg(ManagerBasedRLEnvCfg):
    # Scene settings
    scene: G1Crawl2ShambleSceneCfg = G1Crawl2ShambleSceneCfg(num_envs=4096, env_spacing=4.0)
